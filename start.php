...
$t = new Tmux();
$tmux = $t->get();
$patch = (isset($tmux->tmux_patch)) ? $tmux->tmux_patch : 0;


// Check tmux patch version
if ($patch < 702) {
	exit($c->error("\nYour tmux is not up to date. Please update.\nphp ${DIR}test/DB_updates/patchTmux.php\n"));
}
...
function window_FixNames($tmux_session)
{
	...
	exec("tmux selectp -t 0; tmux splitw -t $tmux_session:1 -v -p 50 'printf \"\033]2;RemoveCrap\033\"'");
	exec("tmux selectp -t 0; tmux splitw -t $tmux_session:1 -h -p 50 'printf \"\033]2;PreDB_Hash_Decrypt\033\"'");
	exec("tmux selectp -t 1; tmux splitw -t $tmux_session:1 -v -p 50 'printf \"\033]2;RequestID\033\"'");
    exec("tmux selectp -t 3; tmux splitw -t $tmux_session:1 -h -p 50 'printf \"\033]2;PrehashUpdate\033\"'");
}
